...
def find_or_create_skill(db: Session, name: str, complexity: str = "Intermediate") -> LookupSkillResponse:
    ...
    # Get next skill_id
    result = db.execute(text("SELECT COALESCE(MAX(skill_id), 0) + 1 FROM tbl_cp_mskills"))
    next_id = result.scalar()
    
    # Get language_id for "General"
    result = db.execute(text("SELECT language_id FROM tbl_cp_mlanguages WHERE LOWER(language_name) = 'general'"))
...
def find_or_create_language(db: Session, language_name: str) -> LookupLanguageResponse:
    ...
    result = db.execute(text("SELECT COALESCE(MAX(language_id), 0) + 1 FROM tbl_cp_mlanguages"))
    next_id = result.scalar()
...
def find_or_create_interest(db: Session, name: str) -> LookupInterestResponse:
    ...
    result = db.execute(text("SELECT COALESCE(MAX(interest_id), 0) + 1 FROM tbl_cp_minterests"))
    next_id = result.scalar()
...
def find_or_create_certification(
    ...
    result = db.execute(text("SELECT COALESCE(MAX(certification_id), 0) + 1 FROM tbl_cp_mcertifications"))
    next_id = result.scalar()
...
def find_or_create_college(db: Session, college_name: str) -> LookupCollegeResponse:
    ...
    # Get next college_id
    result = db.execute(text("SELECT COALESCE(MAX(college_id), 0) + 1 FROM tbl_cp_mcolleges"))
    next_id = result.scalar()
...
def find_or_create_course(
    ...
    # Get next course_id
    result = db.execute(text("SELECT COALESCE(MAX(course_id), 0) + 1 FROM tbl_cp_mcourses"))
    next_id = result.scalar()
